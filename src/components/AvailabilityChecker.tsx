...
export default function AvailabilityChecker({
  ...
  const checkAvailability = async () => {
    try {
      ...
      setError(error.message || "An error occurred while checking availability");
    } finally {
      setIsChecking(false);
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded p-4 bg-gray-50">
            ...
            <ul className="space-y-2">
              {Object.entries(availabilityResult.participantAvailability).map(([email, isAvailable]) => (
                <li key={email} className="flex items-center">
                  ...
                  <span>{email}: {isAvailable ? "Available" : "Has conflicts"}</span>
                </li>
              ))}
            </ul>
          </div>
