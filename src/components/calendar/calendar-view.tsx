...
export function CalendarView({ view, currentDate, events = [] }: CalendarViewProps) {
  ...
  // This would fetch events from Nylas in a real implementation
  const fetchEvents = async () => {
    // Mock events for demonstration
    // In a real app, this would call your backend API that integrates with Nylas
    return [
      {
        id: '1',
        title: 'Team Meeting',
      ...
      <div className="week-view h-[600px] overflow-y-auto">
          ...
          {DAYS_OF_WEEK.map((day, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-sm rounded-full w-7 h-7 flex items-center justify-center mx-auto mt-1 ${isSameDay(date, new Date()) ? 'bg-blue-500 text-white' : ''}`}>
                  {format(date, 'd')}
                </div>
              </div>
            );
        ...
        <div className="grid grid-cols-8">
          <div className="hours-column">
            {HOURS.map((hour) => (
              <div key={hour} className="time-label h-16 text-right pr-2 text-gray-500 text-sm border-b border-gray-200">
                {hour === 0 ? '12 AM' : hour < 12 ? `${hour} AM` : hour === 12 ? '12 PM' : `${hour - 12} PM`}
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-7 gap-px">
        ...
        {calendarDays.map((day, i) => {
          ...
          return (
            <div 
              key={i} 
              className={`min-h-[100px] p-2 border border-gray-200 ${isCurrentMonth ? 'bg-white' : 'bg-gray-50 text-gray-400'} ${isToday ? 'bg-blue-50' : ''}`}
            >
              <div className={`text-right ${isToday ? 'bg-blue-500 text-white rounded-full w-7 h-7 ml-auto flex items-center justify-center' : ''}`}>
                {format(day, 'd')}
              </div>
